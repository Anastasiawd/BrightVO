...
class Attention(nn.Module):
    ...
    def forward(self, x, illu_attn_trans):
        B, N, C = x.shape
        illu_attn = illu_attn_trans
        illu_attn = rearrange(illu_attn, 'b n (h d) -> b h n d', h=self.num_heads)
        illu_attn_s = illu_attn.permute(2,1,0,3)
        if self.with_qkv:
        ...
        if v.shape[2] != illu_attn.shape[2]:
            v_without_cls = v_without_cls * illu_attn_s
            v = torch.cat((cls_token_v.unsqueeze(2), v_without_cls), dim=2)
        else:
            v = v * illu_attn
...
class Block(nn.Module):
    ...
    def forward(self, x, B, T, W, illu_fea):
        ...
        if self.attention_type in ['space_only', 'joint_space_time']:
            ...
            res_temporal = self.drop_path(self.temporal_attn(self.temporal_norm1(xt), illu_attn_trans=illu_fea.permute(1,0,2)))
            ...
            res_spatial = self.drop_path(self.attn(self.norm1(xs), illu_attn_trans=illu_fea.permute(1,0,2)))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,  img_size=(224, 224), patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        n_feat = 64
        self.estimator = Brightness_Estimator(n_feat)
        self.illu_embed = nn.Conv2d(n_feat, embed_dim, kernel_size=patch_size, stride=patch_size)
    ...
    def forward_features(self, x, illu_fea):
        B = x.shape[0]
        x, T, W = self.patch_embed(x)

        illu_fea = rearrange(illu_fea, 'b c t h w -> (b t) c h w')
        illu_fea = self.illu_embed(illu_fea)
        illu_fea = illu_fea.flatten(2).transpose(1, 2)
    ...
    def forward(self, images):
        ...
        br_fea_dim = 64
        illu_feas = torch.zeros(b, br_fea_dim, T, int(h), int(w), device="cuda:0", dtype=torch.float)
        illu_maps = torch.zeros(b, c, T, int(h), int(w), device="cuda:0", dtype=torch.float)
        lit_up_imgs = torch.zeros(b, c, T, int(h), int(w), device="cuda:0", dtype=torch.float)
        for i in range(images.size(2)):
            ...
            lit_up_img = self.gamma * image * illu_map + image
            lit_up_img = lit_up_img.unsqueeze(2)
            illu_map = illu_map.unsqueeze(2)

            illu_feas[:, :, i:i+1, :, :] = illu_fea
            illu_maps[:, :, i:i+1, :, :] = illu_map
            lit_up_imgs[:, :, i:i+1, :, :] = lit_up_img
            
        x = lit_up_imgs
        x_inp= self.forward_features(x, illu_feas)
        x = self.head(x_inp)
        return x
